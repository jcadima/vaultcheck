...
class P001Test extends TestCase
{
    ...
    {
        $this->tmpDir  = sys_get_temp_dir() . '/vaultcheck_p001_' . uniqid();
    ...
    public function test_fires_when_env_is_world_readable(): void
    {
        chmod($this->envFile, 0644); // world-readable
        clearstatcache();

        $context = $this->makeContext($this->tmpDir);
        $results = (new P001_WorldReadable())->run($context);

        $this->assertSame(1, $results->count());
        $findings = iterator_to_array($results->getIterator());
        $this->assertSame('P001', $findings[0]->checkId);
        $this->assertSame(Finding::SEVERITY_CRITICAL, $findings[0]->severity);
    }

    public function test_clean_when_env_is_not_world_readable(): void
    {
        chmod($this->envFile, 0600); // owner read/write only
        clearstatcache();
